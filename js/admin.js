...
import { delegate, lockBodyScroll } from './ui/jquery-ui.js'
...
function renderAdminShell(context, user, route) {
  ...
  document.body.innerHTML = `<div class="admin-app"><aside class="admin-sidebar" data-admin-sidebar><a class="admin-brand" href="/admin"><span class="admin-brand-mark">B</span><span><strong>bworiey</strong><small>Control room</small></span></a><nav class="admin-nav" aria-label="Admin navigation">${menu.map(([key, label]) => `<a class="${route.name === key ? 'is-active' : ''}" href="/admin${key === 'dashboard' ? '' : `/${key}`}" data-admin-nav="${key}">${escapeHtml(label)}<span>↗</span></a>`).join('')}</nav><button class="admin-logout-link" id="admin-logout" type="button">Keluar</button></aside><button class="admin-sidebar-overlay" type="button" data-admin-menu-close aria-label="Tutup navigasi admin"></button><main class="admin-main"><header class="admin-topbar"><button class="admin-menu-toggle" type="button" data-admin-menu-toggle aria-expanded="false" aria-controls="admin-sidebar">Menu</button><span>${formatDate(new Date())} · ${escapeHtml(context.profile?.full_name || user?.email || 'Admin')}</span><a href="/" target="_blank" rel="noopener">Lihat portfolio ↗</a></header><div id="admin-content"></div></main></div>`
  document.querySelector('[data-admin-sidebar]')?.setAttribute('id', 'admin-sidebar')
  const closeAdminMenu = () => { document.body.classList.remove('admin-menu-open'); document.querySelector('[data-admin-menu-toggle]')?.setAttribute('aria-expanded', 'false'); lockBodyScroll(false) }
  const openAdminMenu = () => { document.body.classList.add('admin-menu-open'); document.querySelector('[data-admin-menu-toggle]')?.setAttribute('aria-expanded', 'true'); lockBodyScroll(true) }
  delegate(document, 'click', '[data-admin-menu-toggle]', event => { event.currentTarget.getAttribute('aria-expanded') === 'true' ? closeAdminMenu() : openAdminMenu() }, 'bworieyAdmin')
  delegate(document, 'click', '[data-admin-menu-close], [data-admin-nav]', closeAdminMenu, 'bworieyAdmin')
  delegate(document, 'keydown', 'body', event => { if (event.key === 'Escape') closeAdminMenu() }, 'bworieyAdmin')
